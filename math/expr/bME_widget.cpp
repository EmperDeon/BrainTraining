...
BME::Wgt::Wgt() {
 QVBoxLayout *l_main = new QVBoxLayout;
	...
	l_time = new QLabel("0:00.00"); l_time->setObjectName("math_time");
...
void BME::Wgt::showNext() {
	if(correct + wrong >= 20){
  t_time->stop();
		l_time->setText("0:00.00");
		l_expr->setStyleSheet("color: black");
		l_expr->setText(QString(tr("<div align=\"center\" style=\"font: normal normal 18pt \"Fantasque Sans Mono\";\">"
...
void BME::Wgt::tick() {
 if(time > 1){
	 time--;
	 l_time->setText(QString("%1:%2").arg(0).arg((float)time/100, 2, 'f', 1, '0'));
